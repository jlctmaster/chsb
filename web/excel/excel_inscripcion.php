<?php
  ...
  INNER JOIN educacion.tperiodo p ON i.codigo_periodo = p.codigo_periodo";
	...
	$tituloReporte = "Listado de las Inscripciones";
	$titulosColumnas = array('Código', 'Descripción', 'Fecha Inicio', 'Fecha Culminación', 'Fecha Cierre', 'Estatus');
	...
	while ($row = $mysql->Respuesta($query)){
		$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A'.$i, $row['codigo_inscripcion'])
		->setCellValue('B'.$i, $row['descripcion'])
		->setCellValue('C'.$i, $row['fecha_inicio'])
		->setCellValue('D'.$i, $row['fecha_fin'])
		->setCellValue('E'.$i, $row['fecha_cierre'])
	...
	$objPHPExcel->getActiveSheet()->setTitle('Inscripcion');
	...
	header('Content-Disposition: attachment;filename="Listado Inscripcion.xlsx"');
