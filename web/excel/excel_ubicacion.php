<?php
  ...
  INNER JOIN general.tambiente a ON u.codigo_ambiente = a.codigo_ambiente";
	...
	$tituloReporte = "Listado de las Ubicaciones";
	$titulosColumnas = array('Código', 'Ubicación', 'Ambiente', 'Ubicación Principal', 'Item Defectuoso', 'Estatus');
	...
	while ($row = $mysql->Respuesta($query)){
		$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A'.$i, $row['codigo_ubicacion'])
		->setCellValue('B'.$i, $row['descripcion'])
		->setCellValue('C'.$i, $row['ambiente'])
		->setCellValue('D'.$i, $row['ubicacionprincipal'])
		->setCellValue('E'.$i, $row['itemsdefectuoso'])
	...
	$objPHPExcel->getActiveSheet()->setTitle('Listado Ubicaciones');
	...
	header('Content-Disposition: attachment;filename="Listado Ubicaciones.xlsx"');
