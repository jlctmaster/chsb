<?php
  ...
  b.nro_serial||' '||b.nombre AS item,da.cantidad,CASE a.estatus when '1' then 'ACTIVO' when '0' then 'DESACTIVADA' end as estatus
  ...
  LEFT JOIN bienes_nacionales.tbien b ON da.codigo_item = b.codigo_bien";
	...
	$tituloReporte = "Listado de las Asignaciones";
	$titulosColumnas = array('Código', 'Fecha','Responsable','Item' ,'Cantidad','Estatus');
	...
	while ($row = $mysql->Respuesta($query)){
		$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A'.$i, $row['codigo_asignacion'])
		->setCellValue('B'.$i, $row['fecha_asignacion'])
		->setCellValue('C'.$i, $row['responsable'])
		->setCellValue('D'.$i, $row['item'])
	...
	$objPHPExcel->getActiveSheet()->setTitle('Listado Asignaciones');
	...
	header('Content-Disposition: attachment;filename="Listado Asignaciones.xlsx"');
