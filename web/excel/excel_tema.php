<?php
	...
	$mysql = new Conexion();
	$sql = "SELECT codigo_tema, descripcion, 
			(CASE estatus WHEN '1' THEN 'ACTIVO' ELSE 'DESACTIVADO' END) AS estatus 
			FROM biblioteca.ttema ORDER BY codigo_tema ASC";
	...
	$tituloReporte = "Tema del Libro";
	$titulosColumnas = array('Código', 'Tema', 'Estatus');
	...
	while ($row = $mysql->Respuesta($query)){
		$objPHPExcel->setActiveSheetIndex(0)
		->setCellValue('A'.$i, $row['codigo_tema'])
	...
	$objPHPExcel->getActiveSheet()->setTitle('Temas');
	...
	header('Content-Disposition: attachment;filename="Temas.xlsx"');
