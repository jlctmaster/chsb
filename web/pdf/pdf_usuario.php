...
class clsFpdf extends FPDF {
  ...
  public function Header(){
    ...
    $this->SetFont('Arial','B',10);
    $this->Cell($avnzar);
    $this->Cell($anchura*3+5,$altura,'USUARIO',1,0,'C',$color_fondo);
    $this->Cell($anchura*6,$altura,'NOMBRE PERSONA',1,0,'C',$color_fondo); 
    $this->Cell($anchura*3+5,$altura,'PERFIL',1,0,'C',$color_fondo);
...
if($pgsql->Total_Filas($data)!=0){
 ...
 while($usuario=$pgsql->Respuesta($data)){
    $lobjPdf->Row(array($usuario['nombre_usuario'],
    $usuario['persona'],
