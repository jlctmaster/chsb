<?php
  ...
  class clsFpdf extends FPDF {
   ...
   $this->Cell(0,6,'LISTADO DE LOS TRAYECTOS',0,1,"C");
   $this->Ln(8);
     ...
     $this->SetFillColor(0,0,140); 
      ...
      $this->Cell($anchura*2+6,$altura,'PERÍODO',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'FECHA INICIO',1,0,'L',$color_fondo);
      $this->Cell($anchura*3,$altura,'FECHA FIN',1,0,'L',$color_fondo); 
      $this->Cell($anchura*2+6,$altura,'LAPSO',1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,26,30,30,26,26));
  require_once("../class/class_bd.php");
  $pgsql=new Conexion();
    $sql="SELECT p.codigo_periodo,p.descripcion,p.fecha_inicio,p.fecha_fin,p.estatus, l.lapso AS lapso,
    (CASE p.estatus WHEN '1' THEN 'ACTIVO' ELSE 'DESACTIVADO' END) AS estatus FROM educacion.tperiodo  as p
