...
class clsFpdf extends FPDF {
	...
	public function Header(){
		...
		$this->SetTextColor(0,0,0);
		$this->Cell($anchura*2,$altura,'Cód. Reconstrucción:  ',0,1,'L',$color_fondo);
		$this->SetXY( $r1 + ($r2-$r1)/1.5, $y1+16 );
		$this->SetFont('Arial','',10);
		$this->SetTextColor(0,0,0);
		$this->Cell($anchura,$altura,$fila['codigo_recuperacion'][0],0,1,'R',$color_fondo);
		$this->Ln(5);
		$this->SetFont('Arial','BU',12);
		$this->SetTextColor(0,0,0);
		$this->Cell($avnzar); 
		$this->Cell($anchura*16,$altura,"Reconstrucción de Bienes",0,1,'C',$color_fondo);
	}
...
$sql="SELECT r.codigo_recuperacion,TO_CHAR(r.fecha,'DD/MM/YYYY') AS fecha,
	...
	WHERE r.esrecuperacion='N' AND r.codigo_recuperacion =".$pgsql->comillas_inteligentes($_GET['p1']);
$i=-1;
$data=$pgsql->Ejecutar($sql);
if($pgsql->Total_Filas($data)!=0){
	...
	$lobjPdf->SetFont('Arial','B',10);
	$lobjPdf->SetTextColor(0,0,0);
	$lobjPdf->Cell($anchura*2,$altura,'Bien a Reconstruir: ',0,0,'R',$color_fondo);
	$lobjPdf->Cell($avnzar);
	$lobjPdf->SetFont('Arial','',9);
	$lobjPdf->SetTextColor(0,0,0); 
