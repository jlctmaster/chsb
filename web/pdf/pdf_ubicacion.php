...
class clsFpdf extends FPDF {
  ...
  public function Header(){
    ...
    $this->Cell($anchura*4,$altura,'UBICACIÓN',1,0,'C',$color_fondo); 
    $this->Cell($anchura*3,$altura,'AMBIENTE',1,0,'C',$color_fondo);
    $this->Cell($anchura*5,$altura,'¿UBICACIÓN. PRINCIPAL?',1,0,'C',$color_fondo);
    $this->Cell($anchura*5,$altura,'¿ITEMS DEFECTUOSOS?',1,0,'C',$color_fondo);  
...
if($pgsql->Total_Filas($data)!=0){
  ...
  while($tperfil=$pgsql->Respuesta($data)){
    $lobjPdf->Row(array(
    ucwords($tperfil['codigo_ubicacion']),
    ucwords($tperfil['descripcion']),
    ucwords($tperfil['ambiente']),
    ucwords($tperfil['ubicacionprincipal']),
    ucwords($tperfil['itemsdefectuoso']),
