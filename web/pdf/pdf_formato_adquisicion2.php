...
class clsFpdf extends FPDF {
  ...
  public function Header(){
    ...
    $this->Image("../images/cintillo.jpg" , 25 ,10, 170 , 20, "JPG" ,$_SERVER['HTTP_HOST']."/CHSB/web/"); 
	$this->Ln(25);
		$this->SetFont('Arial','BU',12);;
		$this->Cell(0,6,'ADQUISICIÓN',0,1,"C");
		$this->Ln(5);
		...
		$this->SetFont('Arial','',8); 
		$this->Cell($avnzar); 
		$this->Cell($anchura*2,$altura,'RIF.: J000000000',0,1,'L',$color_fondo);
		$this->Cell($avnzar);
		$this->Cell($anchura*2,$altura,'AQUÍ VA LA DIRECCIÓN',0,1,'L',$color_fondo);
		$this->Cell($avnzar);
  ...
  public function Footer(){
	...
	$lobjPdf->SetWidths(array(30,10,68));
	...
	if($pgsql->Total_Filas($data)!=0){
		...
		while($rows=$pgsql->Respuesta($data)){
			...
			$filas['organizacion'][]=$rows['rorganizacion'];
		...
		$lobjPdf->Row(array('Cod. Adquisición','Fecha Adquisición','Organización','Responsable','Item'),false);
		$lobjPdf->SetFont("arial","",9);
		$lobjPdf->Cell($avnzar);
		for($i=0;$i<count($filas['codigo_adquisicion']);$i++){
			$lobjPdf->Row(array(
			$filas['codigo_adquisicion'][$i],
			$filas['fecha_adquisicion'][$i],
			$filas['organizacion'][$i],			
			$filas['responsable'][$i],
			$filas['item'][$i]),false);
			$lobjPdf->Cell($avnzar);         
		}
