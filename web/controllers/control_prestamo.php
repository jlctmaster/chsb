...
if(isset($_POST['cantidad_a_prestar']))
  $cantidad=trim($_POST['cantidad_a_prestar']);
...
if($lOpt=='Registrar'){
      ...
      if($prestamo->EliminarPrestamos()){
        if(isset($_POST['ejemplar']) && isset($_POST['ubicacion']) && isset($_POST['cantidad'])){
          if($prestamo->InsertarPrestamos($_SESSION['user_name'],$_POST['ejemplar'],$_POST['ubicacion'],$_POST['cantidad'])){
            $mov_inventario->fecha_movimiento($fecha_prestamo);
  ...
  if($confirmacion==1){
    ...
    $_SESSION['datos']['mensaje']="¡El Préstamo ha sido registrada con éxito!";
...
if($lOpt=='Modificar'){
  ...
  if($confirmacion==1){
    $prestamo->Transaccion('finalizado');
    $_SESSION['datos']['mensaje']="¡El Préstamo ha sido modificada con éxito!";
...
if($lOpt=='Desactivar'){
  ...
  if($prestamo->Desactivar($_SESSION['user_name']))
    ...
    $confirmacion=0;
  if($confirmacion==1){
    $_SESSION['datos']['mensaje']="¡El Préstamo ha sido desactivada con éxito!";
    header("Location: ../view/menu_principal.php?prestamo&Opt=3&codigo_prestamo=".$prestamo->codigo_prestamo());
  }else{
